class FormHandler {
  ...
  constructor() {
    ...
    this.validateDocBtn = document.getElementById('validate-doc-btn') as HTMLButtonElement;
    this.initEvents();
  }
  ...
  private initEvents() {
    this.dirBtn.onclick = async () => {
      const dir: string = await (window as any).electronAPI.selectDirectory();
    ...
    this.docInput.addEventListener('input', () => {
      ...
      if (doc.length === 11) {
        ...
        this.docError.textContent = this.validarCNPJ(doc) ? '' : 'CNPJ inválido';
      } else {
    ...
    this.saveBtn.onclick = () => {
      ...
      (window as any).electronAPI.saveFormData(data);
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const data = await (window as any).electronAPI.loadFormData();
  if (data) {
